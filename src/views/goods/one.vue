<template>
  <div>
    ...
    <el-table :data="data">
      ...
      <el-table-column prop="imgs" label="类型图标" align="center">
...
const rules = reactive<FormRules>({
  ...
  img: [{ required: true, message: "请输入类型图标", trigger: "blur" }]
});
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(valid => {
    if (valid) {
      if (title.value == "新增") {
        axios({
          url: "https://c2c.kuxia.top/pcapi/goods/type_add",
          params: {
            name: rule.name,
            img: rule.img
          }
...
const delet = id => {
  if (id != 1) {
    ElMessageBox.confirm("是否确认删除", {
      ...
      .then(() => {
        axios({
          url: "https://c2c.kuxia.top/pcapi/goods/type_del",
          params: {
            id: id
          }
        }).then(src => {
